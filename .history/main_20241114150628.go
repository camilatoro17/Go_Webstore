...
func main() {
	...
	e.GET("/products", func(ctx echo.Context) error {
		...
		// Ensure the exact template name is used here
		if err := ctx.Render(http.StatusOK, "products.templ", data); err != nil {
			log.Printf("Error rendering template: %v", err)
			return ctx.String(http.StatusInternalServerError, "Error rendering page")
		}
		return nil
	})

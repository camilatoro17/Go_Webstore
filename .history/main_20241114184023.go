...
func main() {
	...
	e.POST("/add_car", func(ctx echo.Context) error {
		...
		// Retrieve each form field individually
		product.Name = ctx.FormValue("name")
		product.Image = ctx.FormValue("image")

		// Parse price and quantity as they are numbers
		...
		// Handle the inactive checkbox
		inactive, _ := strconv.Atoi(ctx.FormValue("inactive")) // Assuming "inactive" is sent as "0" or "1"
	...
	e.PUT("/update_car/:id", func(ctx echo.Context) error {
		...
		inactive, err := strconv.Atoi(ctx.FormValue("inactive"))
		if err != nil {
			inactive = 0 // Default to 0 if parsing fails
		}
		product.Inactive = inactive
		product.ID = int64(id)

		fmt.Printf("Received for update: Name=%s, Image=%s, Quantity=%d, Price=%.2f, Inactive=%d\n",
			product.Name, product.Image, product.QuantityInStock, product.Price, product.Inactive)

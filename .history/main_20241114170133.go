...
func main() {
	...
	e.POST("/add_product", func(ctx echo.Context) error {
		var product types.Product
		if err := ctx.Bind(&product); err != nil {
			return ctx.String(http.StatusBadRequest, "Invalid request payload")
		}
		
		if err := db.AddProduct(conn, product); err != nil {
			return ctx.String(http.StatusInternalServerError, "Error adding product")
		}
		return ctx.JSON(http.StatusOK, map[string]string{"message": "Product added successfully"})
	})
	
	e.PUT("/update_product/:id", func(ctx echo.Context) error {
		id := ctx.Param("id")
		var product types.Product
		if err := ctx.Bind(&product); err != nil {
			return ctx.String(http.StatusBadRequest, "Invalid request payload")
		...
		if err := db.UpdateProduct(conn, product); err != nil {
			return ctx.String(http.StatusInternalServerError, "Error updating product")
		}
		return ctx.JSON(http.StatusOK, map[string]string{"message": "Product updated successfully"})
	})
	
	e.DELETE("/delete_product/:id", func(ctx echo.Context) error {
		id := ctx.Param("id")
		if err := db.DeleteProduct(conn, id); err != nil {
			return ctx.String(http.StatusInternalServerError, "Error deleting product")
		}

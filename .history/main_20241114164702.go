...
func main() {
	...
	e.POST("/add-car", func(ctx echo.Context) error {
		...
		_, err := conn.Exec(query, product.Name, product.Image, product.Price, product.QuantityInStock, product.Inactive)
		if err != nil {
			e.Logger.Errorf("Error adding product: %v", err)
			return ctx.JSON(http.StatusInternalServerError, map[string]string{"message": "Error adding product"})
		}

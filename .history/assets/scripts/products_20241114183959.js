...
function updateCar() {
    ...
    // Explicitly add 'inactive' field if it’s unchecked
    const isInactive = $("#inactive").is(":checked") ? 1 : 0;
    data.push({ name: "inactive", value: isInactive });

    console.log("Data being sent for update:", data); // Log data to confirm

    $.ajax({
        type: "PUT",
        url: `/update_car/${carId}`,
        data: $.param(data), // Convert to URL-encoded format
        success: function(response) {
            console.log("Car updated successfully:", response);
            loadProductList(); // Reload list

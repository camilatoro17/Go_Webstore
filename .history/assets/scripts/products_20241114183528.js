...
function updateCar() {
    ...
    const data = $("#product-form").serializeArray(); // use serializeArray to modify inactive value if unchecked

    // Check if 'inactive' is checked and update data accordingly
    let inactiveStatus = data.find(field => field.name === "inactive");
    if (!inactiveStatus) {
        data.push({ name: "inactive", value: 0 }); // If not found, add inactive with value 0
    }

    $.ajax({
        type: "PUT",
        url: `/update_car/${carId}`,
        data: $.param(data), // convert back to serialized format
        success: function(response) {

...
func GetAllProducts(conn *sql.DB) ([]types.Product, error) {
	stmt, err := conn.Prepare("SELECT id, product_name, image_name, price, in_stock, inactive FROM product")
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&product.ID, &product.Name, &product.Image, &product.Price, &product.QuantityInStock, &product.Inactive); err != nil {
...
func AddCar(conn *sql.DB, product types.Product) error {
    fmt.Printf("Received product to add: Name=%s, Image=%s, Price=%f, QuantityInStock=%d, Inactive=%d\n",
        product.Name, product.Image, product.Price, product.QuantityInStock, product.Inactive)

    stmt, err := conn.Prepare("INSERT INTO product (product_name, image_name, price, in_stock, inactive) VALUES (?, ?, ?, ?, ?)")
    if err != nil {
        return fmt.Errorf("AddCar: prepare: %v", err)
    }
    defer stmt.Close()

    _, err = stmt.Exec(product.Name, product.Image, product.Price, product.QuantityInStock, product.Inactive)
    if err != nil {
        return fmt.Errorf("AddCar: exec: %v", err)
    }
...
func DeleteCar(conn *sql.DB, productID int64) error {
    var count int
    err := conn.QueryRow("SELECT COUNT(*) FROM orders WHERE product_name = (SELECT product_name FROM product WHERE id = ?)", productID).Scan(&count)
    if err != nil {
        return fmt.Errorf("DeleteCar: query check orders: %v", err)
    }

    if count > 0 {
        return fmt.Errorf("DeleteCar: cannot delete product with existing orders")
    }

    // Delete car
    _, err = conn.Exec("DELETE FROM product WHERE id = ?", productID)
    if err != nil {
        return fmt.Errorf("DeleteCar: exec delete: %v", err)
    }
    return nil
}

func UpdateCar(conn *sql.DB, product types.Product) error {
    _, err := conn.Exec(
        "UPDATE product SET product_name = ?, image_name = ?, price = ?, in_stock = ?, inactive = ? WHERE id = ?",
        product.Name, product.Image, product.Price, product.QuantityInStock, product.Inactive, product.ID,
    )
    if err != nil {
        return fmt.Errorf("UpdateCar: exec: %v", err)
    }
    return nil
}
